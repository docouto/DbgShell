...
namespace MS.Dbg.Formatting.Commands
{
    ...
    public abstract class FormatBaseCommand< TView > : FormatBaseCommand where TView : class
    {
        ...
        private Dictionary< Type, TView > m_propViews;
            ...
            else if( _UsingPropertyProperty )
            {
                ...
                foreach( object propThing in Property )
                {
                    if( propThing is string propName )
                    {
                        ...
                        {
                            hasWildcard = true;
                            m_propViews = new Dictionary< Type, TView >();
                            break;
                        }
                    }
                }
     ...
     // private Type _GetViewTypeCacheKeyForInputObject()
     // {
     //     Type t = InputObject.GetType();
     //     var pso = InputObject as PSObject;
     //     if( null != pso )
     //     {
     //         t = pso.BaseObject.GetType();
     //     }

     //     // Some types, like PSCustomObject don't necessarily have a fixed set of
     //     // properties. In that case, if we have wildcards in our calculated view, we
     //     // have no choice but to check each one.
     //     if( t == typeof( PSCustomObject ) )
     //     {
     //         t = null;
     //     }
     ...
     // }
        ...
        protected override void ProcessRecord()
        {
         ...
         // if( null != m_propViews )
         // {
         //     // We are in the "-Property has a wildcard" case. Before generating a new
         //     // view for the current object, let's check our cache.

         //     Type t = _GetViewTypeCacheKeyForInputObject();
         //     if( (null == t) || !m_propViews.TryGetValue( t, out m_view ) )
         //     {
         //         m_view = _GenerateViewFromPropertyProperty();
         //         if( null != t )
         //         {
         //             m_propViews.Add( t, m_view );
         //         }
         //     }
         // }
